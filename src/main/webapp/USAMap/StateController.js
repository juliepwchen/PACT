...
define(['app-config', 'd3', 'queue', 'topojson'], function (app, d3, queue, topojson) {
    app.register.controller('StateController', ['$rootScope', '$scope', '$location', '$http',
    ...
    var config = {width: 600, height: 400, color1: "#6ea865", color2: "#1d7533", state: "Provider State", coveredCharges: " Average Covered Charges ", totalPayments:" Average Total Payments " }
    ...
    /* d3.tsv("properties/hipreplacement.tsv", function(data) {
      var cost_data = {}
    ...
    var divs = d3.select("div.tooltips");
    /* console.log("DIVS= "+ divs.length) */

    /* if (divs.length === 0) {
        var div = d3.select("#canvas-svg")
        div = d3.select("#canvas-svg")
            .append("div")
            .attr("class", "tooltips")
            .style("opacity", 0);
    } else {
    ...
    d3.json("properties/hipreplacement.json", function(data) {
      /* var cost_data = {} */
      data.forEach(function(d) {
        /* console.log("State=" + d[config.state] ); */
        ...
        svg.selectAll('.state-path')
              ...
              .attr("centroid", function(d) {
                ...
                if (cost_data[id_name_map[d.id].code]) {
                  /* console.log("cost_data[id_name_map[d.id].code] =" + cost_data[id_name_map[d.id].code]); */

                  console.log("In .State-Path, Centroid[1] BEFORE = " + centroid[1]);
                  centroid[1] = centroid[1] - cost_data[id_name_map[d.id].code].charge / 1000;
                  console.log("In .State-Path, Centroid[1] AFTER = " + centroid[1]);

                  cost_data[id_name_map[d.id].code].centroid = centroid;

                  /* console.log("Cost_data Centroid =" + cost_data[id_name_map[d.id].code].centroid); */
                }
              });
          ...
          for (var state in cost_data) {
              g.append("rect")
                  ...
                  .on("mouseover", function(d) {
                      ...
                      console.log("In MouseOver, Centroid[0] = " + centroid[0]);
                      console.log("In MouseOver, Centroid[1] = " + centroid[1]);
